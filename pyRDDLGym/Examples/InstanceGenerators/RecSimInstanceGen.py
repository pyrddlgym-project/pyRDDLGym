...
class RecSimInstanceGenerator(InstanceGenerator):
  ...
  def sample_instance(self, params: Dict[str, object]) -> Dict[str, object]:
    ...
    provider_clusters = self.init_random_provider_clusters(provider_disp, provider_fan_out, num_provider_clusters, num_topics)
    user_locs = self.init_random_user_points(provider_clusters, num_users, user_stddev)
    ...
    for i, provider in enumerate(providers):
      nonfluents[f'PROVIDER-COMPETENCE({provider}, f1)'] = provider_clusters[i][0]
      nonfluents[f'PROVIDER-COMPETENCE({provider}, f2)'] = provider_clusters[i][1]
    providers = ['pn'] + providers # Add dummy provider for total ordering purposes.
    ...
    return {
      'objects': {'feature': features, 'item': items, 'consumer': users, 'provider': providers},
...
params = [
    ...
    # difficulty is controlled by horizon and number and diversity users and providers.
    {'provider_dispersion': 64., 'provider_fan_out': 2, 'num_provider_clusters': 8,
     'num_users': 100, 'docs_per_cluster': 5, 'user_stddev':  20.,
     'horizon': 100, 'discount': 1.0},


    {'provider_dispersion': 64., 'provider_fan_out': 2, 'num_provider_clusters': 25,
     'num_users': 500, 'docs_per_cluster': 5, 'user_stddev':  20.,
