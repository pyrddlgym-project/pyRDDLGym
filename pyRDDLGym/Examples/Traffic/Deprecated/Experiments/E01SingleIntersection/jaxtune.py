...
tuner.tune_slp(key=key,
               init_state_subs=init_state_subs)
...
for callback in gen:
    ...
    if jnp.isnan(train_return[-1]) or jnp.isnan(test_return[-1]):
        raise RuntimeError
    ...
    unclipped_grads = callback['grad']['advance']
    print(jnp.min(unclipped_grads), jnp.max(unclipped_grads))
    grads.append(callback['updates']['advance'])
    grads_min.append(jnp.min(grads[-1]))
    grads_max.append(jnp.max(grads[-1]))
    print(grads[-1][:10])
    #sleep(1)
    #print(grads[-1])
    print(jnp.sum(callback['action']['advance'][:,:,0], axis=1))
    actions.append(callback['action']['advance'][0,:,0].tolist())


fig, ax = plt.subplots(1, 2)
fig.set_size_inches(14, 7)

X = jnp.arange(0, nepochs+step-1, step)
...
with open(f'img/webster_{timestamp}.json', 'w') as file:
    json.dump({'step': step,
               'nepochs': nepochs,
