...
for callback in gen:
    ...
    if train_return[-1] > best_train:
        print(callback['params'])
        for k, v in callback['params'].items():
            print(k)
            print(jax.nn.sigmoid(policy_hyperparams[k] * v))
        best_train = train_return[-1]
        print(best_train, test_return[-1])
