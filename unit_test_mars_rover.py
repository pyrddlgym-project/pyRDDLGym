...
def mars_rover_update(state, action):
    ...
    new_mineral_harvested_m2 = state['mineral_harvested_m2'] or (
        (math.sqrt((state['pos_x_d1'] - -8) ** 2 + (state['pos_y_d1'] - -8) ** 2) < 6 and action['harvest_d1'])
    ...
    reward = -(u_x_d1 **2 + u_y_d1 ** 2) + -(u_x_d2 **2 + u_y_d2 ** 2) + \
        (8 if ((math.sqrt((state['pos_x_d1'] - 5) ** 2 + (state['pos_y_d1'] - 5) ** 2) < 6 and action['harvest_d1'])
        or (math.sqrt((state['pos_x_d2'] - 5) ** 2 + (state['pos_y_d2'] - 5) ** 2) < 6 and action['harvest_d2'])) else 0) \
        + (5 if ((math.sqrt((state['pos_x_d1'] - -8) ** 2 + (state['pos_y_d1'] - -8) ** 2) < 6 and action['harvest_d1'])
        or (math.sqrt((state['pos_x_d2'] - -8) ** 2 + (state['pos_y_d2'] - -8) ** 2) < 8 and action['harvest_d2'])) else 0) \
